...
/** Evaluate value, gradient, and metric-contracted laplacian for one orbital.
 *
 *  The six Hessian components remain in registers until their contraction with
 *  symGGt. The VGL path writes only the value, three gradients, and laplacian.
 *
 *  @param symGGt Symmetrized metric tensor in HESS00, HESS01, HESS02, HESS11,
 *                HESS12, HESS22 order.
 *  @param val_grads_lapl Structure-of-Array output in strides of out_offset.
 *  @param out_offset Stride between value and gradient fields.
 *  @param lapl_offset Offset from the value field to the laplacian field.
 */
template<typename T>
