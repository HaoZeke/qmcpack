...
struct DiracDeterminantBatched<PL, VT, FPVT>::DiracDeterminantBatchedMultiWalkerResource : public Resource
{
  ...
  /** scratch for the accept path, reused rather than rebuilt per move.
   *
   * mw_accept_rejectMove runs once per particle per step, so building these inside it
   * is three heap allocations for every particle of every walker move. The contents
   * are device pointers and engine references that are rewritten each call anyway.
   */
    ...
    bool safe_to_delay) const
{
  ...
  RefVectorWithLeader<UpdateEngine> engine_list(wfc_leader.det_engine_);
  engine_list.reserve(nw);
  // reused from the crowd resource; see the declaration for why
