...
std::pair<int, int> EinsplineSetBuilder::obtainMemoryAttributes(const xmlNodePtr cur)
{
  ...
  // % and * share a precedence level and associate left to right, so the product has
  // to be parenthesised: without it the test reads (node_comm_size % distributed_ranks)
  // * shared_ranks, which is zero whenever distributed_ranks divides the rank count,
  // and shared_ranks never takes part in the check at all.
